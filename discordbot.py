...
def dcchannel(name: str, type = 'textchannel'):
    if type == 'textchannel':
        namee = (name.replace('.', 'ㆍ'))
        namee = (namee.replace('A', '𝗔'))
        namee = (namee.replace('B', '𝗕'))
        namee = (namee.replace('C', '𝗖'))
        namee = (namee.replace('D', '𝗗'))
        namee = (namee.replace('E', '𝗘'))
        namee = (namee.replace('F', '𝗙'))
        namee = (namee.replace('G', '𝗚')) 
        namee = (namee.replace('H', '𝗛'))
        namee = (namee.replace('I', '𝗜'))
        namee = (namee.replace('J', '𝗝'))
        namee = (namee.replace('K', '𝗞'))
        namee = (namee.replace('L', '𝗟'))
        namee = (namee.replace('M', '𝗠'))
        namee = (namee.replace('N', '𝗡'))
        namee = (namee.replace('O', '𝗢'))
        namee = (namee.replace('P', '𝗣'))
        namee = (namee.replace('Q', '𝗤'))
        namee = (namee.replace('R', '𝗥'))
        namee = (namee.replace('S', '𝗦'))
        namee = (namee.replace('T', '𝗧'))
        namee = (namee.replace('U', '𝗨'))
        namee = (namee.replace('V', '𝗩'))
        namee = (namee.replace('W', '𝗪'))
        namee = (namee.replace('X', '𝗫'))
        namee = (namee.replace('Y', '𝗬'))
        namee = (namee.replace('Z', '𝗭'))
        namee = (namee.replace(' ', '_'))
        namee = (namee.replace('\n', '_'))
        namee = (namee.replace(' | ', 'ㅣ'))
        namee = (namee.replace(' l ', 'ㅣ'))
        namee = (namee.replace(' I ', 'ㅣ'))
    elif type == 'voicechannel':
        namee = (name.replace('A', '𝗔'))
        namee = (namee.replace('B', '𝗕'))
        namee = (namee.replace('C', '𝗖'))
        namee = (namee.replace('D', '𝗗'))
        namee = (namee.replace('E', '𝗘'))
...
async def on_ready():
    ...
    if os.path.isfile(file):
        ...
        f.write(f'\n[ {now.hour}:{now.minute}:{now.second} ] 봇이 시작됨')
        f.close()
    else:
        f = open(file, 'w', encoding='utf-8')
        f.write(f'[ {now.hour}:{now.minute}:{now.second} ] 봇이 시작됨')
        f.close()
...
async def log(content):
    ...
    if os.path.isfile(file):
        f = open(file, 'a', encoding='utf-8')
        f.write(f'\n[ {now.hour}:{now.minute}:{now.second} ] ' + content)
        f.close()
    else:
        f = open(file, 'w', encoding='utf-8')
        f.write(f'[ {now.hour}:{now.minute}:{now.second} ] ' + content)
        f.close()
    
    return f'[ {now.hour}:{now.minute}:{now.second} ] ' + content
...
async def cola(ctx):
    if isinstance(ctx.channel, discord.abc.PrivateChannel) == True:
        ...
        await log(str(ctx.author) + ' 님이 이스터 에그 발견!')
    else:
        await ctx.send("잉? 뭔말이에요?")
...
async def unite(ctx, user : discord.Member, *, name):
    if isinstance(ctx.channel, discord.abc.PrivateChannel) != True:
        if ctx.guild.id in useguild:
            ...
            ch = await ctx.guild.create_text_channel(namee, category=category)
            ro1 = ctx.guild.get_role(868175381230944267)
            ro2 = ctx.guild.get_role(868175984422191157)
            ro3 = ctx.guild.get_role(868177254730051654)
...
async def 공지역할지급(ctx):
    ...
    if isinstance(ctx.channel, discord.abc.PrivateChannel) != True:
        if ctx.guild.id in useguild:
            ...
            role1 = ctx.guild.get_role(873054728588759060)
            for i in ctx.guild.members:
                try:
                    ...
                    result1 = result1 + str(i) + '(알 수 없는 오류), '
                    pass
            
            await log(result1 + '님에게 ' + str(role) + ' 역할과' + str(role1) + ' 역할을 지급했습니다')
